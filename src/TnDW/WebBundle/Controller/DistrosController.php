...
//use TnDW\WebBundle\Logic\WolframAlphaEngine;
...
class DistrosController extends Controller {
    ...
    public function distroAction($distro) {
        ...
        $result = pg_query($ph, "select name, shortname, id from backing_distribution where id in (" .
                ...
                "	select id from backing_basedondistro where shortname = '" . $distrodata->shortname . "' limit 1))");
        ...
        # countryurl
        $countryurls = $this->getCountryOutlineWolfram($distrodata->origin);
    ...
    private function getCountryOutline($origin) {
        if (preg_match('/,/', $origin))
            return "";
        else if (preg_match('/Isle\ of\ Man/i', $origin))
            return "";
        else if (preg_match('/usa/i', $origin))
            return "http://0.tqn.com/d/geography/1/0/9/H/usa3.jpg";
    ...
    private function getCountryOutlineWolfram($origin, $parameter = "shape") {
        $shapes = array();
        $url_base = 'http://api.wolframalpha.com/v1/query.jsp';
        $appid = $this->container->getParameter('wolframAlphaAppId');
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        foreach (preg_split("/,/", $origin) as $origin) {
            $url = $url_base . "?appid=" . urlencode($appid) . "&input=" . urlencode($origin . " " . $parameter) . "&format=image";
            curl_setopt($ch, CURLOPT_URL, $url);
            $response = new \SimpleXMLElement(curl_exec($ch));
            $data = $response->xpath("//pod[@title='Result']/subpod/img");
            $shapes[] = $data[0]->attributes()->src;
        }
